...
| 0xffd4 | runtime | - | streaming.c | JUSTIFIED (#46) — `CPTRXCNF4` DIVB2, now written with a COMPUTED value (`0x03` for 44.1/48 kHz, `0x01` for 88.2/96 kHz) in `streaming_set_rate()`, which is why the scanner reports it as `runtime` rather than as an immediate. Both values are stock's: ÷4 is the boot value at Rev 20 fcn.0x08CB @ 0x0929 / Rev 22 fcn.0x07EC @ 0x084A, and ÷2 is the mode-5 value at Rev 20 fcn.0x0728 @ 0x07A0 / Rev 22 @ 0x077E. The doubling mechanism is the divider, not the ACG word: the synthesizer's range is 12–25 MHz (datasheet §2.2.6) and 48 kHz already runs it at 24.576 MHz, so no 96 kHz frequency word exists. Measured 2x on hardware when this register was wrongly left at 0x01 (IEPDCNTX1 = 96 samples/frame vs 48). Bracketed by CPTEN off/on per stock's mode-5 ordering; stock's following `VECINT = 0` is deliberately NOT copied — see the comment at the site. |
| 0xffdd | runtime | - | streaming.c | JUSTIFIED (#46) — `CPTCNF4` DIVB, the playback-side twin of the row above and written from the same computed value in the same bracket. ÷4 is stock's boot value (Rev 20 fcn.0x08CB @ 0x0911, Rev 22 fcn.0x07EC @ 0x0832); the ÷2 case is **NOVEL**, because stock's only runtime divider write is mode 5's, which halves the receive divider alone. A rate is a property of both directions, so halving one without the other would put playback and capture on different frame rates. |
| 0xff9a | assign | 0x57 | usb.c | JUSTIFIED (#46) — `OEPBSIZ2` = 0x57 = **696 B** for the PLAYBACK buffer, where stock writes 0x50 = 640 B (Rev 20 fcn.0x0970 @ 0x09AE, Rev 22 fcn.0x0891 @ 0x08CF). A DELIBERATE departure. This value was 0x48 (576 B) in build 0x002A, matching the capture buffer; the two are now sized separately because the two directions do not want the same thing from a circular buffer. **Capture** needs the wrap frame-aligned: per §6.4.4.4 an isochronous BSIZ sizes a single *circular* buffer, so a size that is not a whole number of frames moves the wrap point by the remainder every frame and splices samples — measured on build 0x0029, which carried the correct tone at the correct amplitude (goertzel 0.0226 vs 0.0227 at 48 kHz) with **full-scale samples spliced in**, peak 1.0002 against 0.0456. **Playback** needs slack instead: the DMA drains what the host has finished writing, so a buffer of exactly one frame leaves the read pointer nowhere to be that the write pointer is not. 576 B is exactly one frame at 96 kHz, and build 0x002A measured playback there at **−97.5 dBFS** — dead silent — with OEPCNF2=0xC5, alt 2 held for the whole run, DMAEN set, OEPDCNTX2 reporting 96 samples/frame arriving and no host-side error. 696 B = 116 samples = 1.21 frames at 96 kHz and 2.42 at 48 kHz. It is a multiple of **6** (one stereo 24-bit sample) as well as of 8 (the BSIZ granularity): a size that is not a whole number of samples puts the read pointer mid-sample on every wrap, which is exactly what Rev 22's SOF watchdog (fcn.0x0D58, streaming_sof()) exists to detect. Stock's own 640 B is likewise not a whole number of frames OR samples at 48 kHz, with that watchdog as the backstop — so this is stock's arrangement resized for 96 kHz, not a new one. 696 + 576 = 1272 of the 1288 B region, which is the ceiling: two whole playback frames at 96 kHz would need 1152, and 1152 + 576 = 1728 > 1288. |
| 0xff61 | assign | 0x9b | usb.c | JUSTIFIED (#46) — `IEPBBAX1` = 0x9B, i.e. capture buffer base **0xFCD8**, where stock writes 0x94 = 0xFCA0 (Rev 20 fcn.0x0970 @ 0x09A8, Rev 22 fcn.0x0891 @ 0x08C9). Consequence of the 0xff9a row above and not an independent decision: the capture buffer starts wherever the playback buffer ends. It was 0x8C (0xFC60) while playback was 576 B and moved to 0x9B when playback grew to 696 B. Layout: playback 0xFA20-0xFCD7 (696 B), capture 0xFCD8-0xFF17 (576 B), contiguous inside the 0xFA20-0xFF27 region, leaving 16 B (0xFF18-0xFF27) below the setup-packet block. BBAX carries address bits 10:3, so both bases stay 8-byte aligned as the register requires; 696 is a multiple of 8, so the capture base inherits the alignment. |
| 0xff62 | assign | 0x48 | usb.c | JUSTIFIED (#46) — `IEPBSIZ1` = 0x48 = **576 B** for the CAPTURE buffer, where stock writes 0x50 = 640 B (Rev 20 fcn.0x0970 @ 0x09B4, Rev 22 fcn.0x0891 @ 0x08D5). Until build 0x002B this register took the same value as OEPBSIZ2 and the two shared one constant; they are now independent, so this row records the capture side on its own. 576 B is a whole number of frames at BOTH rates — exactly 2 at 48 kHz and exactly 1 at 96 — which is what capture needs and what fixed the spliced full-scale samples build 0x0029 measured. Note the consequence for the emitted code: stock's four-write block reuses the accumulator across the two BSIZ writes (`90 ff 9a 74 50 f0 90 ff 62 f0`, Rev 20 0x09AB / Rev 22 0x08CC), which is why #162 could match it byte-for-byte. Two different constants cannot reuse A, so mboxfw now emits a second `MOV A,#imm` here and the block is no longer byte-identical to stock's. That is the intended cost of sizing the directions separately. |
