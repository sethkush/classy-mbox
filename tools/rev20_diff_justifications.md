...
| 0xff5b | assign | 0x03 | streaming.c | **MBOXFW_ONLY, #186 stage 2** — `IEPDCNTX2` (0xFF5B) = 3, arming the 3-byte feedback packet. TI does exactly this in `SoftPll.c::softPll`, which arms this register and `IEPDCNTY2` (0xFF5F) together with the same 3 — the closest thing to a reference this write has. |
