...
| 0xffc2 | rmw | - | mboxfw | JUSTIFIED — `I2CDATI` (0xFFC2), the I²C receive-data register, read once per byte by `eeprom_read_seq()` (`dst[i] = I2C_RX;`) per TI `I2c.c::I2CAccess` line 111. Reported as **rmw** rather than a read because SDCC keeps DPTR loaded across the surrounding `I2C_STA` accesses, so the scanner sees read-then-store on one DPTR — the same codegen artefact already documented for `USBCTL` at 0xfffc above. There is no store to 0xFFC2 in the source, and none in the listing. **Rev 20 reads the same register on its own EEPROM read path**, so this is MBOXFW_ONLY only in the sense that stock's read is reached from code mboxfw does not mirror; the register, the direction and the TI reference are identical. Present only in builds carrying `eeprom_read_seq()` — `MBOX_SERIAL_EEPROM` (#221) and `MBOX_PROVISION` (#226). |
...
| 0xffc0 | or | 0x02 | rev20 | **NO LONGER AN OMISSION — corrected 2026-08-16 (#226).** `I2CSTA` (0xFFC0) bit 1 = STOP_READ, set by stock when it READS the EEPROM. mboxfw now sets it too, on the same register with the same meaning, in `eeprom_read_seq()` (TI `I2c.c::I2CAccess` line 136 — armed on the second-to-last byte so the STOP lands on the last one). Its two siblings named below, `I2CDATO` = 0xFF and `I2CADR` = 0xA1, are likewise emitted again, and `I2CDATI` (0xFFC2) has its own row above. All four returned with #221's EEPROM-provisioned serial, which reads the record at boot. **What follows is the historical text, true between 2026-08-05 and #221 and kept because the reasoning it records — that a removed read path is safe because recovery never depended on it — is still the reason the WRITE path is the one that must never be broken:** mboxfw no longer reads it: `eeprom_read_byte()` and `eeprom_smoke_test()` were removed on 2026-08-05 with the boot-button DFU trigger, their only caller. The WRITE path is unaffected and still matches — `TLM_REQ_ENTER_DFU` invalidates the header checksum through `eeprom_write_byte()`, which is the DFU trigger in daily use. Two sibling writes went the same way and need no separate rows because they are read-only-path too: `I2CDATO` (0xFFC1) = 0xFF, the dummy write that clocks a read, and `I2CADR` (0xFFC3) = 0xA1, the read address for the EEPROM at slave 0x50. Recovery from a non-enumerating image is the SDA-short bootstrap in BRICK_LOG.md, which needs no firmware cooperation. |
...
| 0xff53 | runtime | - | usb.c | **RETIRED at #228 — the row is kept so the address is not re-derived.** `IEPDCNTX3` was the byte count for EP3 IN, the UAC1 status interrupt endpoint. That endpoint is GONE: #207 added it so a front-panel press could tell the host its Selector had moved, and #228 took the analog front-end choice off the host's Selector entirely, leaving nothing on this device that changes a host-visible control behind the host's back. #227's finding — that this register must be armed to 0x80 (the NAK flag) or the endpoint answers every poll with a zero-length packet — is preserved in FINDING_227 and must be re-applied if an asynchronous control is ever added. No build emits this write now. |
