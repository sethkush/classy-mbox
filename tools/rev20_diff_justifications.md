...
| 0xff53 | runtime | - | usb.c | **MBOXFW_ONLY, #227** — `IEPDCNTX3`, the byte count for EP3 IN, the UAC1 status interrupt endpoint. Written twice: `0x80` at configure time and the packet length from `usb_status_notify()`. The top bit is the NAK flag, so `0x80` means "nothing to report" — without it the endpoint answers every interrupt poll with a zero-length packet for the life of the device, which is the defect the EP0 comment at `IEPDCNTX0` (0xFF6B) `= 0x80` already records against itself ("could confuse strict hosts"). #207 added this endpoint without copying that line. Stock ships no status endpoint at all, so there is no address to mirror; UAC1 §3.7.1.2 defines the endpoint and its two-byte status word. |
