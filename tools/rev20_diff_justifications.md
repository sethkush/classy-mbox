...
| 0xffb1 | runtime | - | mboxfw | ~~GLOBCTL `\|= 0x02` in hw_init~~ **REMOVED 2026-07-29 after a hardware measurement: it makes the device SILENT ON USB.** Build 0x0010 (present) never attaches; 0x0011 (removed) attaches in 7 s, bisected back to back. Stock sets it, but stock runs hw init BEFORE USB and mboxfw does the reverse (#47). Original note follows: Emitted as a runtime RMW (SDCC routes the mask through ar7 when it fuses this with the adjacent MEMCFG (0xFFB0) RMW), which is why the pattern reads `runtime` rather than an immediate. Mirrors stock's boot `GLOBCTL = 0x06` (Rev 20 0x08FE, Rev 22 0x081F); RMW per #48 reaches the same value from the boot ROM's 0x04. See the retraction above. |
