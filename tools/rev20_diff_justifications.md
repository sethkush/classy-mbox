...
| 0xffd5 | assign | 0xac | hw_init.c | **#161 EXPERIMENT, build 0x001D.** `CPTRXCNF3` bit 2 is BYOR (datasheet §6.5.4.12, address FFD5h). This matches stock's BOOT value (Rev 20 fcn.0x08CB @0x0923, Rev 22 @0x0844) and diverges from stock's RUNNING 0xA8, exactly as 0xffde does. History: mboxfw ran 0xAC playback / 0xA8 capture and BOTH directions measured correct, which the datasheet does not explain — §6.5.4.3 and §6.5.4.12 define BYOR with word-for-word identical text, and the slot geometry is identical (CPTCNF2 = 0xE5 is 24 data bits in 32-cycle slots; CPTRXCNF2 = 0x25 is the same 24-in-32 with TSL0L = 00b, "same as other time slots", rather than an explicit 11b), so there is no lane shift to account for it. §6.5.4.12's "only used in I2S Mode 5" is NOT an escape: CPTCNF1 = 0x0D has MODE(2:0) = 101b, which is mode 5, so the register is live by the datasheet's own condition. This build sets 0xAC to ask whether the RX BYOR bit reaches our capture path at all. Clean audio => it does not, there is no asymmetry, and stock's symmetric 0xA8/0xA8 is consistent. Garbage capture with intact playback => receive genuinely wants BYOR CLEAR while transmit wants it SET, and 0xA8 returns with a measurement behind it. Single-variable: playback is at its proven-good 0xAC. |
