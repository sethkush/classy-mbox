...
| 0xff5b | runtime | - | streaming.c | **MBOXFW_ONLY, #186 stage 2, widened by #215** — `IEPDCNTX2` (0xFF5B), the feedback packet's byte count. TI does exactly this in `SoftPll.c::softPll`, which arms this register and `IEPDCNTY2` (0xFF5F) together — the closest thing to a reference this write has. It was `assign 0x03` until 2026-08-16 and is now `runtime`, sourced from `g_fb_count`, which still initialises to `AUDIO_FEEDBACK_LEN` = 3. **The shipped behaviour is unchanged**; the value merely became a variable so `TLM_REQ_FB_TUNE` can re-arm it from a bench host. That exists because #211 measured the endpoint emitting **9** bytes while this register says 3, and whether the register governs the length at all cannot be settled with a compile-time constant without one flash and one 2 km round trip per value tried. Note the pattern is `runtime` in EVERY build, not only `MBOX_FB_TUNE=1` ones: the variable is unconditional so this baseline does not depend on which flags were passed, which would make the audit gate's answer a function of the build command. |
| 0xff5f | runtime | - | streaming.c | **MBOXFW_ONLY, #186 stage 2** — `IEPDCNTY2`, armed to the same 3 alongside the X count, again following `SoftPll.c::softPll`. Shows as runtime rather than an immediate because SDCC carries the constant in the accumulator across the pair. |
