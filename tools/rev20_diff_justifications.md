...
| 0xfffd | assign | 0x9f | rev20 | SAFE_OMIT (with corrected reasoning) — Rev 20 ACTUALLY uses USBIMSK = 0x9F in both boot init (rev20_flat.asm 0x09FE-0x0A03) and VEC_RSTR handler (0x0F7E-0x0F80). The 2026-07-23 note previously in this row was doubly wrong: it claimed Rev 20's boot USBIMSK was 0xE5 at 0x091A, but 0x091A actually writes 0xE5 to CPTCNF4 (0xFFDF), not USBIMSK. Ground truth verified 2026-07-25 by re-reading rev20_flat.asm bytes. mboxfw's and safety_net's 0xE5 (TI UsbEng.c:647) is a DELIBERATE architectural divergence from Rev 20: we dispatch SETUP via STPOW interrupt (bit 5 of USBIMSK); Rev 20 uses a polled EP0 model that doesn't need STPOW. Do NOT change ours to 0x9F — it would silence our SETUP handler. See safety_net main.c:417-419 and mboxfw usb.c comment for the architectural note. |
