...
| 0xff61 | assign | 0x60 | streaming.c | mboxfw uses buffer layout EP1_IN_BUF_ADDR=0xFB00 → IEPBBAX1 = (0xFB00-0xF800)/8 = 0x60. Rev 20 chose 0x94 (buffer @ 0xFCA0). Different buffer placements in TAS1020A shared memory. **SAFE_OMIT** — mboxfw's layout is self-consistent (0xFA10 EP0-OUT, 0xFA18 EP0-IN, 0xFB00 EP1-IN, 0xFC00 EP2-OUT, all non-overlapping, all outside boot-ROM DFU packet area 0xFC00-0xFDFF... wait). ⚠ Verify no overlap with Rev-20-established DFU packet windows before flash: EP2_OUT_BUF_ADDR=0xFC00 with size 0x100 uses 0xFC00-0xFCFF; if boot ROM re-enters DFU it may want to reuse that region. Justified for first-flash safety-net where audio isn't active. Full mboxfw flash: re-audit. |
