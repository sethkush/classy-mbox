...
| 0xff9a | assign | 0x48 | usb.c | JUSTIFIED (#46) — `OEPBSIZ2` = 0x48 = **576 B**, where stock writes 0x50 = 640 B (Rev 20 fcn.0x0970 @ 0x09AE, Rev 22 fcn.0x0891 @ 0x08CF). A DELIBERATE departure, and an alignment fix rather than a size cut. Per datasheet §6.4.4.4 an isochronous BSIZ sizes a single **circular** buffer, so a buffer that is not a whole number of frames moves its wrap point by the remainder every frame and frames straddle it at a different offset each time. 640 is a whole number of frames at **neither** rate this firmware supports: 640/288 = 2.22 at 48 kHz, 640/576 = 1.11 at 96 kHz. 576 is exactly 2 frames at 48 kHz and exactly 1 at 96 kHz. Stock is not wrong for stock — it has no 96 kHz mode, and at 48 kHz its 352 B of slack keeps the drain ahead of the wrap so the misalignment never surfaces. It surfaced immediately at 96 kHz: build 0x0029 measured capture carrying the correct tone at the correct amplitude (goertzel 0.0226 vs 0.0227 at 48 kHz) with **full-scale samples spliced in** — peak 1.0002 against 0.0456 on the same tone at 48 kHz. Both buffers at 576 B total 1152 B inside the 1288 B region, leaving 0xFEA0-0xFF27 free. Bases move accordingly: 0xFA20 playback, 0xFC60 capture (was 0xFCA0). |
| 0xff61 | assign | 0x8c | usb.c | JUSTIFIED (#46) — `IEPBBAX1` = 0x8C, i.e. capture buffer base **0xFC60**, where stock writes 0x94 = 0xFCA0 (Rev 20 fcn.0x0970 @ 0x09A8, Rev 22 fcn.0x0891 @ 0x08C9). Consequence of the 0xff9a row above and not an independent decision: the playback buffer shrank from 640 to 576 B, so the capture buffer that follows it starts 64 B earlier. The two 576 B buffers occupy 0xFA20-0xFC5F and 0xFC60-0xFE9F, contiguous, inside the 0xFA20-0xFF27 region, leaving 0xFEA0-0xFF27 (136 B) free — more headroom below the setup-packet block than stock's layout left. BBAX carries address bits 10:3, so both bases stay 8-byte aligned as the register requires. |
