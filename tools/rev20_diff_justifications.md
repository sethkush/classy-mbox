...
| 0xff62 | assign | 0x40 | streaming.c | IEPBSIZ1 = 0x40 → 512-byte buffer (EP_BSIZE(0x200)). Fixed 2026-07-23: was 0x20 (256B), smaller than a 48 kHz stereo 24-bit frame (288B) → every 48 kHz packet truncated. **OPEN, 2026-07-30 (#162).** Stock writes 0x50 = **640 bytes** in both revs (Rev 20 @0x09AE reused at 0x09B4; Rev 22 @0x08CF reused at 0x08D5), so this is still a CHANGED divergence, not a resolved one. The stated reason — "to fit below the 0xFF00 SFR boundary" — is false: there is no boundary at 0xFF00. The setup-packet window starts at 0xFF28 (datasheet Table 6-2) and stock's capture buffer runs 0xFCA0-0xFF1F, using all of it. §6.4.4.4: for **isochronous** endpoints BSIZ sizes a *single circular buffer*, so 512 B cannot hold two 288-byte USB frames at 48 kHz where 640 B can. Un-cited divergence inside the #147 subsystem. See FINDING_147_cport_and_ep_buffer_divergences.md §3. |
...
| 0xff9a | assign | 0x40 | streaming.c | OEPBSIZ2 = 0x40 → 512-byte buffer, same rationale as 0xff62 — and the same **OPEN (#162)** status: stock writes 0x50 (640 B) here too. |
...
| 0xffb1 | and_not | 0xfe | rev20 | **WRONG ON BOTH COUNTS — corrected 2026-07-31.** mboxfw does not set `\|= 0x01`; it never writes GLOBCTL at all. And bit 0 is not a "USB-enable bit": §6.5.7.4 names it **CPTEN**, codec port enable. Stock's clear/set pairs bracket every C-port reconfiguration. See the 0xffb1 row above and #168. |
...
| 0xffd5 | assign | 0xa8 | hw_init.c | **NOT A DIVERGENCE — reclassified 2026-07-30 (see FINDING_147_cport_and_ep_buffer_divergences.md §2).** CPTRXCNF3 = 0xA8 is stock's own RUNNING value. Stock's 0xAC is a boot-init value that survives only until the host's first SET_CONFIGURATION: work code 1 (posted @0x0293) runs cmd1_apply_clock_mode, which stops both DMAs, drops GLOBCTL CPTEN, and calls the helper at Rev 20 0x0FF4 / Rev 22 0x0FE2 — that helper writes ONE value to BOTH CPTCNF3 (0xFFDE) and CPTRXCNF3 (0xFFD5) and raises CPTEN. Its 0xAC call site (Rev 20 @0x034A) is gated on bit 0x0A = IRAM 0x21.2, which the Keil init table @0x0F9C zeroes and which no instruction in either image ever sets (scanned for D2/C2/B2/92 on the bit and for every direct byte write to 0x21) — unreachable. The live site is @0x0355 with A = 0xA8. The datasheet's I2S Mode 5 example for this exact geometry (NTSL=2, TSL0L=11b, TSLL=101b, BPTSL=100b, CSYNCL=1, CSYNCP=0, DDLY=1) also lists BYOR = 0. The old justification's endianness chain is void: Linux reports stock as S24_3BE *while stock runs BYOR=0*, so BYOR=0 cannot be the little-endian setting — §6.5.4.12 says BYOR=0 preserves the received order, which for MSB-first I2S is big-endian. Which BYOR value mboxfw's declared S24_3LE actually needs was OPEN (#161); it is now MEASURED. Capture at BYOR=0 decodes correctly as little-endian (FINDING_147_capture_works_analog_path_does_not.md), which FALSIFIES the endianness chain above in the direction we have hard data for: that chain predicts BYOR=1 for LE and capture is correct at BYOR=0. Treat the Linux-quirk inference as void for BOTH directions, not just this one. |
...
| 0xffde | assign | 0xa8 | hw_init.c | **#161 EXPERIMENT, build 0x001C.** `CPTCNF3` bit 2 is BYOR. mboxfw ran 0xAC here while running 0xA8 at CPTRXCNF3 — asymmetric, which stock NEVER is: the helper at Rev 20 0x0FF4 / Rev 22 0x0FE2 writes ONE accumulator to BOTH 0xFFDE and 0xFFD5, so stock is 0xAC/0xAC at boot and 0xA8/0xA8 running, and cannot differ between the two. Our asymmetry was residue: the capture line was updated to stock's running value during #147 while this one was left at stock's boot value pending a measurement. The 2026-08-03 sweep proved 0xAC is CORRECT but not UNIQUELY correct — only one value was ever tried. This build clears BYOR to test whether the transmit-path bit does anything in this configuration. Clean audio => it is inert, keep 0xA8 and the divergence from stock disappears; broken audio => BYOR-TX is real and 0xAC returns with a measurement behind it. |
| 0xffde | assign | 0xac | rev20 | **SAFE_OMIT.** 0xAC is stock's BOOT value only (Rev 20 fcn.0x08CB @0x090B, Rev 22 @0x0831). Stock overwrites it with 0xA8 at the first SET_CONFIGURATION (Rev 20 @0x0358 via fcn.0x0FF4, Rev 22 @0x035E via fcn.0x0FE2) and never returns to it — the 0xAC runtime site at Rev 20 @0x034A is gated on IRAM 0x21.2, which nothing in either image sets. Omitting it matches stock's steady state rather than diverging from it, exactly as for 0xffd5. |
