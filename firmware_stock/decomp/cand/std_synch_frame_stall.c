// MATCH: image=rev20 addr=0x02E7 len=3 func=std_synch_frame_stall cflags=--peep-file,firmware_stock/decomp/keil.peep
extern void ep0_stall_both(void);

/* The SYNCH_FRAME arm of the standard-request switch in std_request_dispatch
 * (0x0118). The ?C?CCASE table entry for bRequest = 12 points here; see
 * cand/std_request_dispatch.c for the table.
 *
 * This is not a function in the source, it is one case arm: the only reference
 * to 0x02E7 in the whole image is the table entry, and the arm's body is a
 ...
 *
 * Behaviour: SYNCH_FRAME is refused. EP0 IN and OUT are both stalled
 ...
 */
void std_synch_frame_stall(void) { ep0_stall_both(); }
