// MATCH: image=rev20 addr=0x0970 len=153 func=usb_ep_dma_init cflags=--peep-file,firmware_stock/decomp/keil.peep
/* Endpoint buffer and DMA channel setup. Runs once from main(), immediately
 * after hw_master_init and before USBCTL.CONN is asserted.
 *
...
void usb_ep_dma_init(void) __naked {
    __asm
        ...
        mov   a,#0x80
        movx  @dptr,a
        ; Rev 20 fcn.0x0970 @ 0x09E0 — DMACTL0 = 0x02
        ...
        mov   a,#0x02
        movx  @dptr,a
        ; Rev 20 fcn.0x0970 @ 0x09E6 — DMACTL1 = 0x09
        ; EPDIR=1 (IN) + EPNUM=1.
