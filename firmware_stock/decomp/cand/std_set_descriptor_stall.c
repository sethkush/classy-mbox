// MATCH: image=rev20 addr=0x0299 len=3 func=std_set_descriptor_stall cflags=--peep-file,firmware_stock/decomp/keil.peep
extern void ep0_stall_both(void);

/* The SET_DESCRIPTOR arm of the standard-request switch in std_request_dispatch
 * (0x0118). The ?C?CCASE table entry for bRequest = 7 points here; see
 * cand/std_request_dispatch.c for the table.
 *
 * This is not a function in the source, it is one case arm: the only reference
 * to 0x0299 in the whole image is the table entry, and the arm's body is a
 ...
 *
 * Behaviour: SET_DESCRIPTOR is refused. EP0 IN and OUT are both stalled
 ...
 */
void std_set_descriptor_stall(void) { ep0_stall_both(); }
