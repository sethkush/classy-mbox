...
void streaming_set_rate(unsigned long hz)
{
    ...
    /* WHY THIS IS UNCONDITIONAL, AND STAYS SO.
     *
     * Build 0x004B guarded the whole block on "the clock mode actually
     * changed", reasoning that a host asking for the rate already running
     * disturbs no clock and so needs no bracket. Every bench test passed:
     * repeat captures clean from sample 0, rate changes still reprogramming
     * with the ACG reading 11289.4 and 12287.9 MCLK/frame against nominals.
     *
     * A COLD BOOT KILLED IT. Unit B, first capture after a real power cycle:
     * a standing DC offset of +0.002 rising to +0.007, about -43 dBFS, and it
     * did not decay -- it stayed for the whole power-up. Unit A on the
     * unconditional build, same bench, same minute: -98.1 dBFS with DC at
     * zero.
     *
     * The reason is the one thing the optimisation threw away. If the boot
     * calibration lands while the analog rails are still settling it stores a
     * wrong offset, and stock's recalibration at every stream open is what
     * replaces it. It is not redundant work; it is the self-correction. Skip
     * it and a bad boot calibration becomes permanent, and nothing short of a
     * power cycle clears it.
     *
     * The cost of getting this right is ~183 ms of digital zeros at the top of
     * every capture -- the AK5383's calibration cycle, tRTV = 8960/fs. Stock
     * pays it (Rev 20 0x03BA and Rev 22 0x03BE both LCALL the mode-apply on
     * every stream open) and so do we.
     *
     * A refinement that might buy the silence back without the risk: calibrate
     * on stream CLOSE as well as at boot, so the gap falls after a take rather
     * than at the head of the next one. Untested, and it needs its own
     * cold-boot proof before it goes anywhere near a build. */
