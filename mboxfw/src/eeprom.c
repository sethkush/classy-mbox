 ...
 *
 * DESTINATION IS __data, NOT __xdata, and that is the whole of #226. This
 * board implements NO external RAM: XDATA is backed only in the 0xF800-0xFFFF
 * shared-memory window, so a linker-allocated __xdata buffer (SDCC starts them
 * at 0x0001, because `--xram-size 0x1000` claims 4 KB that does not exist) is
 * dead space -- writes vanish, reads return 0x00. This function was always
 * correct; every byte it read was being thrown into a hole. Reading the EEPROM
 * header at 0x0001 returns 0x12 with the buffer in internal RAM and 0x00 with
 * it in XDATA, same code. Do not widen this back to __xdata.
 *
